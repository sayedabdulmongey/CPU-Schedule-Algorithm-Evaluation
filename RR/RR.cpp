...
int main()
{
    ...
    while (1)
    {
        ...
        if (!gantt_chart.empty() && (gantt_chart.back().remaining_burst_time == 0 || tmp == 0))
        {
            ...
            {
                gantt_chart.push_back(ready_queue.front());
                update_disbatch_time(click, ready_queue.front().name); // p --> excution --> vector [dispatch click]
                ready_queue.pop();
            }
            tmp = q;
        }
        if (gantt_chart.empty() && !ready_queue.empty())
        {
            gantt_chart.push_back(ready_queue.front());
            update_disbatch_time(click, ready_queue.front().name);
            ready_queue.pop();
        }
